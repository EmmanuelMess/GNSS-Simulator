...
from src.conversions import velocity_eci2ecef, position_ecef2eci
...
class Solver:
    ...
    def solve_position(self, satellite_positions_eci, pseudorange, xtol):
        """
        ...
        position_aproximation_ecef = np.array([1, 1, 1], dtype=np.float64)
        ...
        for _ in range(100):
            ...
            pseudorange_approximation = (
                        np.linalg.norm(satellite_positions_eci - position_aproximation_ecef, axis=1)
                        + (gnss_receiver_clock_bias_approximation - self.satellite_clock_bias * scipy.constants.c))

            delta_gnss_pseudorange = pseudorange.copy() - pseudorange_approximation

            delta_satelites = position_aproximation_ecef - satellite_positions_eci
            cs = np.ones((1, satellite_amount), dtype=np.float64)

            G = np.concatenate((delta_satelites / pseudorange_approximation.reshape((-1, 1)), cs.T), axis=1)

            m = (np.linalg.pinv(G) @ delta_gnss_pseudorange.T).reshape(-1)
            gnss_position_delta = m[:3]
            gnss_clock_bias_delta = m[-1]

            position_aproximation_ecef += gnss_position_delta
        ...
        return position_aproximation_ecef, gnss_receiver_clock_bias_approximation, np.linalg.norm(delta_gnss_pseudorange)
    ...
    def solve_velocity(self, satellite_positions_eci, satellite_velocities_eci, direct_doppler, receiver_position_ecef,
        ...
        """
        # TODO add satelite weighting
        receiver_position_eci = position_ecef2eci(receiver_position_ecef, 0.0)
